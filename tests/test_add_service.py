...
from api.services.service_services.add_service import (
    add_service,
    RESERVED_KEYS
)
...
def test_reserved_keys_constant():
    ...
    expected_keys = {
        ...
        "collection", "service_url", "service_type", "health_check_url",
        "documentation_url"
    }
...
class TestAddService:
    ...
    def test_add_service_package_create_error(self, mock_ckan_settings):
        ...
        mock_ckan = MagicMock()
        mock_ckan.action.package_create.side_effect = Exception("Package creation failed")
        mock_ckan_settings.ckan = mock_ckan
    ...
    def test_add_service_resource_create_error(self, mock_ckan_settings):
        ...
        mock_ckan = MagicMock()
        mock_ckan.action.package_create.return_value = {"id": "service-123"}
        mock_ckan.action.resource_create.side_effect = Exception("Resource creation failed")
        mock_ckan_settings.ckan = mock_ckan
    ...
    def test_add_service_optional_service_fields(self, mock_ckan_settings):
        ...
        package_call = mock_ckan.action.package_create.call_args
        extras_dict = {extra["key"]: extra["value"] for extra in package_call[1]["extras"]}
