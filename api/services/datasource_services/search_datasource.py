    ...
    try:
        ...
        for dataset in datasets['results']:
            ...
            for resource in dataset.get('resources', []):
                # Ensure that the resource matches all provided filter
                # conditions
                if (
                    (
                        not resource_url or
                        resource.get('url') == resource_url) and
                    (
                        not resource_name or
                        resource.get('name') == resource_name) and
                    (
                        not resource_description or
                        resource.get('description') == resource_description)
                    and
                    (
                        not resource_format or
                        resource.get(
                            'format').lower() == resource_format.lower())
                ):
                    # Add only those resources that match all conditions
                    matching_resources.append(resource)

            # Include dataset if at least one resource matches all the provided
            # conditions
            if matching_resources:
                ...
                organization_name = dataset.get(
                    'organization', {}).get(
                        'name') if dataset.get('organization') else None
                extras = {
        ...
        if search_term:
            keywords_list = [
                keyword.strip().lower() for keyword in search_term.split(",")]
            results = [
                dataset for dataset in results
                if stream_matches_keywords(dataset, keywords_list)]
...
def stream_matches_keywords(stream, keywords_list):
    ...
    """
    # Convert stream object to JSON string in lowercase for easier keyword
    # matching
