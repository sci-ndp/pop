        ...
        if field.startswith("extras:"):
            ...
            if key == "license_or_otherLicense":
                ...
                if ("license_id" not in document or not document["license_id"]) and (
                    "extras" not in document
                    or "otherLicense" not in document["extras"]
                    or not document["extras"]["otherLicense"]
                ):
                    missing_fields.append("license_id / extras:otherLicense")
            elif (
                "extras" not in document
                or key not in document["extras"]
                or not document["extras"][key]
            ):
            ...
            if (
                "resource" not in document
                or subfield not in document["resource"]
                or not document["resource"][subfield]
            ):
                missing_fields.append(field)
        else:
            # Handle top-level fields like 'title', 'notes', 'tags'
            # We'll check if the corresponding mapped key exists and is not empty
            actual_key = key_mapping.get(
                field, field
            )  # Use mapping if available, else field itself
