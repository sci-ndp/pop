...
@router.api_route(
    "/jupyterhub/{path:path}", methods=[
        "GET", "POST", "PUT", "DELETE", "PATCH", "HEAD", "OPTIONS"
    ]
)
async def proxy_to_jupyterhub(request: Request, path: str = ""):
    """
    Proxy requests to the JupyterHub server,
    this including static assets and other resources
    """
    hub_url = f"http://jupyterhub:8002/{path}"

    # Extract headers from the original request
    # Ensuring no duplicate Host header
    headers = {
        key: value
    ...
    try:
        ...
        if "Location" in response.headers:
            ...
            if original_location.startswith("/"):
                ...
                rewritten_location = original_location.replace(
                    "http://jupyterhub:8002", "/jupyterhub"
                )
            else:
        ...
        return JSONResponse(
            {"error": "Unable to connect to JupyterHub"},
            status_code=502
        )
